...
public class Problem1629 {
    ...
    public char slowestKey(int[] releaseTimes, String keysPressed) {
        ...
        for (int i = 1; i < keysPressed.length(); i++) {
            int cnt = releaseTimes[i] - releaseTimes[i - 1];
//            if (cnt >= max) {
//                if (cnt == max) {
...
//                max = cnt;
//            }
            if(cnt > max || (cnt == max && keysPressed.charAt(i) > res)) {
                res = keysPressed.charAt(i);
                max = cnt;
            }
        }
    ...
    public static void main(String[] args) {
        Problem1629 problem = new Problem1629();
        System.out.println(problem.slowestKey(new int[]{12, 23, 36, 46, 62}, "spuda"));

...
public class Problem1657 {
    ...
    public boolean closeStrings(String word1, String word2) {
        ...
        for (int n : word1Chars) {
            word1Map.put(n, word1Map.getOrDefault(n, 0) + 1);
        }
        for (int n : word2Chars) {
            word2Map.put(n, word2Map.getOrDefault(n, 0) + 1);
        }
        for (int i = 0; i < 26; i++) {
        ...
        for (Map.Entry<Integer, Integer> entry : word1Map.entrySet()) {
            if (!entry.getValue().equals(word2Map.get(entry.getKey()))) {
                return false;
            }
        }
        return true;
